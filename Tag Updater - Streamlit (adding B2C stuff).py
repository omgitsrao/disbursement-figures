...
if mode == "G2P Disbursements":
    ...
    detailed_file = st.file_uploader(
        "Upload Detailed G2P Excel File",
        type=["xlsx", "xls"]
    )
    ...
    if detailed_file and tagging_file:
        ...
        # Load Files
        # -----------------------
        detailed_df = read_file(detailed_file)
        tagging_df = read_file(tagging_file)
        ...
        # -----------------------
        # Select Required Columns (includes category_cps)
        # -----------------------
        ...
        output_name = base + " - Updated.xlsx"

        st.download_button(
            "Download Updated G2P File",
            data=to_excel_download(detailed_df),
    ...
    PROGRAM_CONFIG = {
        "BISP": {
            "revenue_col":     "REVENUE",
            "disb_amt_col":    "total_amt",
            "disb_txn_col":    "total_transactions",
            "collect_amt_col": None,
            "collect_txn_col": None,
        },
        "PSPA": {
            "revenue_col":     "TOTAL_REVENUE",
            "disb_amt_col":    "TRXAMT",
            "disb_txn_col":    "NO_OF_TRANSACTIONS",
            "collect_amt_col": None,
            "collect_txn_col": None,
        },
        "RSU": {
